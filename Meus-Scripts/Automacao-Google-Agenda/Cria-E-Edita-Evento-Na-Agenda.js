      ...
      if (exists != ""){
        try {
          ...
          if (event){
            ...
            // sincroniza a lista de convidados
            const emailsDesejados = [email1, email2, email3]
            .filter(e => e !== "" && e != null)
            .map(e => e.toString().trim().toLowerCase());

const convidadosAtuais = event.getGuestList()
  .map(convidado => convidado.getEmail().toLowerCase());

// remove quem não deveria mais estar
for (const emailAtual of convidadosAtuais){
  if (!emailsDesejados.includes(emailAtual)){
    event.removeGuest(emailAtual);
  }
}

// adiciona quem ainda não está
for (const emailDesejado of emailsDesejados){
  if (!convidadosAtuais.includes(emailDesejado)){
    event.addGuest(emailDesejado);
  }
}
          ...
          Logger.log("Erro ao tentar atualizar." + e);
        }
      }
