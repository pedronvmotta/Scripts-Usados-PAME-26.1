...
function cruzarHorarios() {
  ...
  const HEADER = ['Candidato', 'Data', 'Horário', 'Membro', 'Cargo', 'Email do membro'];
  const novasLinhas = [];
  ...
  if (!existentes.temHeader) {
    ...
    if (paraAdicionar.length > 0) {
      formatarTexto(abaSaida, 2, paraAdicionar.length);
    }
    ...
    formatarTexto(abaSaida, startRow, paraAdicionar.length);
...
function formatarTexto(aba, startRow, numRows) {
  try {
    aba.getRange(startRow, 2, numRows, 2).setNumberFormat('@');
  } catch (e) {
